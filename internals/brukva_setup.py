...
from internals.constants import (CHAT_DISTANCE, HURT_DISTANCE,
                                 MESSAGES_WITH_GUIDS, tilesize)


def distance(pos1, pos2):
    return hypot(abs(pos1[0] - pos2[0]), abs(pos1[1] - pos2[1]))
    ...
    def on_notify_location(location, message):
        ...
        if location in comm.locations:
            ...
            if message_type == "giv":
                ...
                pos = message.split("\n", 1)[0].split(":")[-2:]
                pos = map(int, pos)
            elif message_type == "atk":
                atk_by, atk_with, atk_guid = message[3:].split(":", 2)
                clients = [c for c in comm.locations[location] if
                           c.id == atk_guid]
                if not clients:
                    return
                clients[0]._attacked(atk_by, atk_with)
                return
